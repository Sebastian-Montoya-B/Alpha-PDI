...
plot( x=NULL, y=NULL, xlim=c(0.5,2.5), ylim=c(0,1), ylab="", xlab="")
points(x=rep(1, length(class.gen)), class.gen)
points(x=rep(2, length(class.spe)), class.spe)
for (i in 1:length(class.gen)){
  lines(x=c(1,2), c(class.gen[[i]],class.spe[[i]]) )
}
...
plot(summ_samp$NI, summ_samp$aPDI, log="x")
cor.test(summ_samp$NI, summ_samp$aPDI, method = "spearman", exact=F)
...
gen_fleas2<-alpha_PDI(consumers$Moyynkum, 
                     resources$Moyynkum)$corrected_aPDI
...
frame_col2<-data.frame(consumer=rownames(consumers$Moyynkum), 
                      aPDI=gen_fleas2, color.group=color_group(gen_fleas2))

imat<- graph_from_incidence_matrix(consumers$Turkmenistan, 
                                   directed = F,weighted = TRUE)
imat2<- graph_from_incidence_matrix(consumers$Moyynkum, 
                                   directed = F,weighted = TRUE)


algoritmo<-layout.reingold.tilford(imat, circular=T)
algoritmo2<-layout_with_gem(imat2)
...
V(imat2)$color = V(imat2)$type
V(imat2)$color[which(V(imat2)$color==FALSE)]<-colores[frame_col2$color.group]
V(imat2)$color = gsub(TRUE,"gray",V(imat2)$color)
#V(imat)$color[which(rowSums(consumers$Turkmenistan)<80)]<-alpha("#658c1f",0.7)## Consumers with less than 100 interactions
V(imat2)$size<-V(imat2)$type
V(imat2)$size[which(V(imat2)$size==FALSE)]<-rep(9,nrow(frame_col2))
V(imat2)$size[which(V(imat2)$size==TRUE)]<-log(resources$Moyynkum)
V(imat2)$shape <- c("circle", "rhombus")[V(imat2)$type+1]
E(imat2)$width <- log(E(imat2)$weight/5000)
...
text(x=c(0.25,0.78), y=0.5, cex=2,
     labels=c("(A)\nPercentage of consumers in each network",
              "(B)\nTurkmenistan network"))

### 3.2.2. Second plot
par(mar=c(5,5,2,2), las=1)
x1_xit<-jitter(rep(1, length(class.gen)), factor=5)
x2_xit<-jitter(rep(2, length(class.gen)), factor=5)
plot( x=NULL, y=NULL, xlim=c(0.5,2.5), ylim=c(0,1), ylab="Proportion of consumers in each network", 
      xlab="", xaxt="n", cex.lab=1.8, cex.axis=1.2)

for (i in 1:length(class.gen)){
  lines(x=c(x1_xit[i],x2_xit[i]), c(class.gen[[i]],class.spe[[i]]), col="gray", lty=2)
}

points(x=x1_xit, class.gen, pch=21,col=alpha("#df4f00",0.7), bg=alpha("#df4f00",0.4), cex=1.5)
points(x=x2_xit, class.spe, pch=21,col=alpha("#00918d",0.7), bg=alpha("#00918d",0.4), cex=1.5)
axis(1, at=c(1,2), labels =c("Generalists", 'Specialists'), cex.axis=1.2)

#TernaryPlot(alab="Consumers with\nfewer than 80 recorded interactions \u2192", blab="Generalists \u2192", 
...
par(las=1,mar=c(0,0,0,0))
algoritmo2<-layout_with_fr(imat2)
library(qgraph)

e <- get.edgelist(imat2,names=FALSE)
l <- qgraph.layout.fruchtermanreingold(e,vcount=vcount(imat2), area=8*(vcount(imat2)^2), repulse.rad=(vcount(imat2)^3.1))
plot(imat2,vertex.label.cex=1,vertex.color = V(imat2)$color, 
     vertex.size = V(imat2)$size, edge.curved=.3,layout =l, rescale=T, 
     vertex.label.color="black",vertex.label=NA)
